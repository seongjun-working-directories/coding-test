'''
[문제 013] - https://programmers.co.kr/learn/courses/30/lessons/64061
...
'''
def solution_me(board, moves):
    answer = 0
    
    stack = [
        [board[x][y] for x in range(len(board)) if board[x][y]] for y in range(len(board[0]))
    ]
    ...
    for lane in moves:
        if stack[lane-1]:
            if result and (result[-1] == stack[lane-1][-1]):
                result.pop()
                stack[lane-1].pop()
                answer += 2
            else:
                item = stack[lane-1].pop()
                result.append(item)
    
    return answer

def solution(board, moves):
    lanes = [[] for _ in range(len(board[0]))]  # 각 열에 대한 스택 생성

    for i in range(len(board) - 1, -1, -1):   # board를 역순으로 탐색해 각 열의 인형을 lanes에 추가
