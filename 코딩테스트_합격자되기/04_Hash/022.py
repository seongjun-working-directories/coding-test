'''
[문제 022] - https://programmers.co.kr/learn/courses/30/lessons/42888
...
def solution_me(record):
    ...
    for command in record:
        ...
        if command.startswith("Enter"):
            user_db[keyword[1]] = str(keyword[2])
            answer.append(keyword[1]+"님이 들어왔습니다.")
        elif command.startswith("Leave"):
            answer.append(keyword[1]+"님이 나갔습니다.")
        elif command.startswith("Change"):
            user_db[keyword[1]] = str(keyword[2])
    
    for i in range(len(answer)):
        answer[i] = user_db[answer[i].split("님")[0]] +"님" + answer[i].split("님")[1]
...
def solution(record):
    ...
    # uid 등록을 위해 record를 우선 한번 순회
    for line in record:
        cmd = line.split(" ")
        if cmd[0] != "Leave":
            uid[cmd[1]] = cmd[2]
    
    # 이후 uid를 기반으로 record에 최종 닉네임을 적용하며 다시 순회
    for line in record:
        cmd = line.split(" ")
        if cmd[0] == "Enter":
...
print(solution_me(["Enter uid1234 Muzi", "Enter uid4567 Prodo","Leave uid1234","Enter uid1234 Prodo","Change uid4567 Ryan"]))
