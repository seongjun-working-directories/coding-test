'''
[문제 025] - https://programmers.co.kr/learn/courses/30/lessons/72411
...
'''
itertools의 combinations(조합)와 permutations(순열)

arr = ['A', 'B', 'C']
nPr = itertools.permutations(arr, 2)  # [A, B, C]의 리스트에서 2개의 원소를 골라 순서를 정해 나열
print(list(nPr)) # [('A', 'B'), ('A', 'C'), ('B', 'A'), ('B', 'C'), ('C', 'A'), ('C', 'B')]

arr = ['A', 'B', 'C']
nCr = itertools.combinations(arr, 2)  # [A, B, C]의 리스트에서 2개의 원소를 골라 순서를 생각하지 않고 나열
print(list(nCr))  # [('A', 'B'), ('A', 'C'), ('B', 'C')]


collections의 Counter 객체
...
def solution(orders, course):
    ...
    for c in course:            # 각 코스 요리의 길이에 대해
        menu = []
        ...
        # counter가 있고, counter의 최대값이 1이 아닌 경우 계산 실행
