'''
[문제 017] - https://school.programmers.co.kr/learn/courses/30/lessons/159994
...
def solution(cards1, cards2, goal):
    ...
    queue_one = deque(cards1)
    queue_two = deque(cards2)
    queue_goal = deque(goal)
    
    count = 0
    while queue_goal:
        check = queue_goal.popleft()
        if (queue_one and (queue_one[0] == check)):
            queue_one.popleft()
        elif (queue_two and (queue_two[0] == check)):
            queue_two.popleft()
        else:
            return 'No'
