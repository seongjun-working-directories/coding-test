'''
[문제 029] - https://programmers.co.kr/learn/courses/30/lessons/77486
...
young 은 이 중 10% 에 해당하는 120 원을,
자신을 조직에 참여시킨 추천인인 edward 에게 배분하고 자신은 나머지인 1,080 원을 가집니다.
...
jaimie 의 추천인인 mary 는 2 원을 받지만
이것의 10% 는 원 단위에서 절사하면 배분할 금액이 없기 때문에
mary 는 2 원을 모두 가집니다.
이 상태를 그림으로 나타내면 아래와 같습니다.

그 다음으로 emily 가 칫솔 판매를 통하여 얻은 이익 500 원은 마찬가지의 규칙에 따라
emily 에게 450 원, mary 에게 45 원, 그리고 센터에 5 원으로 분배됩니다.
이 상태를 그림으로 나타내면 아래와 같습니다.

마지막으로, 판매원 mary 는 1,000 원의 이익을 달성하고,
이 중 10% 인 100 원을 센터에 배분한 후 그 나머지인 900 원을 자신이 가집니다.
이 상태를 그림으로 나타내면 아래와 같습니다.
...
지금까지 얻은 이익을 모두 합한 결과를 그림으로 나타내면 아래와 같습니다.
                ...
                center(146)
    john(360)                               mary(958)
                    (108)edward             emily(450)          jaimie(18)
                (0)sam     young(1080)                          tod(180)
...
["john", "mary", "edward", "sam",   ["-", "-", "mary", "edward", "mary",        ["young", "john", "tod", "emily", "mary"]       [12, 4, 2, 5, 10]       [360, 958, 108, 0, 450, 18, 180, 1080]
"emily", "jaimie", "tod", "young"]	"mary", "jaimie", "edward"]
["john", "mary", "edward", "sam",   ["-", "-", "mary", "edward", "mary",        ["sam", "emily", "jaimie", "edward"]            [2, 3, 5, 4]            [0, 110, 378, 180, 270, 450, 0, 0]
"emily", "jaimie", "tod", "young"]  "mary", "jaimie", "edward"]
'''
...
def solution_me(enroll, referral, seller, amount):
    answer = []
    
    # seller와 amount를 사용해서 딕셔너리 제작
    seller_dict = {}
    for i in range(len(seller)):
        if seller[i] not in seller_dict.keys():
            seller_dict[seller[i]] = []
        seller_dict[seller[i]].append(amount[i] * 100)
    
    # enroll과 referral, seller_dict를 기반으로 딕셔너리 제작
    enroll_dict = {}
    enroll_dict["-"] = Node(None)
    for i in range(len(enroll)):
        enroll_dict[enroll[i]] = Node(referral[i])
        if enroll[i] in seller_dict.keys():
            for element in seller_dict[enroll[i]]:
                current_profit = element
                enroll_dict[enroll[i]].profit += ceil(current_profit * 0.9)
                
                start = enroll_dict[enroll_dict[enroll[i]].parent]
                current_profit = current_profit - ceil(current_profit * 0.9)
                while True:
                    if current_profit < 1:
                        break
                    if start.parent is None:
                        start.profit += current_profit
                        break
                    else:
                        start.profit += ceil(current_profit * 0.9)
                        start = enroll_dict[start.parent]
                        current_profit = current_profit - ceil(current_profit * 0.9)
    
    for e in enroll:
        answer.append(enroll_dict[e].profit)
...
def solution(enroll, referral, seller, amount):
    ...
    # print(parent)
    # {
    #   'john': '-', 'mary': '-', 'edward': 'mary', 'sam': 'edward',
    #   'emily': 'mary', 'jaimie': 'mary', 'tod': 'jaimie', 'young': 'edward'
    # }
    ...
    for i in range(len(seller)):
        ...
        while money > 0 and current_name != "-":
            total[current_name] += money - money // 10
