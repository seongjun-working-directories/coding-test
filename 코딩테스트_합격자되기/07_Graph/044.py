'''
[문제 044] - https://school.programmers.co.kr/learn/courses/30/lessons/12978
...
다음은 N = 5, K = 3인 경우의 예시입니다. (예시 1번 확인)
...
def graph_builder(road):
    ...
    for item in road:
        ...
        if u not in road_dict.keys():
            road_dict[u] = {}
            road_dict[u][v] = value
        else:
            if v not in road_dict[u].keys():
                road_dict[u][v] = value
            else:
                if road_dict[u][v] > value:
                    road_dict[u][v] = value
        
        if v not in road_dict.keys():
            road_dict[v] = {}
            road_dict[v][u] = value
        else:
            if u not in road_dict[v].keys():
                road_dict[v][u] = value
            else:
                if road_dict[v][u] > value:
                    road_dict[v][u] = value
    
    return road_dict

def solution_me(N, road, K):
    graph = graph_builder(road)
    ...
    while queue:
        ...
        for v, value in graph[current_node].items():
            cost = current_distance + value
            if distances[v] > cost:
                distances[v] = cost
...
print(solution_me(6, [[1,2,1],[1,3,2],[2,3,2],[3,4,3],[3,5,2],[3,5,3],[5,6,1]], 4))
